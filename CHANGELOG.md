...
- **GitHub repo link** moved from a fixed bottom-left control into the **floating
  toolbar**, immediately after **Export**, with chip styling aligned to Insert /
  Export buttons (`App.tsx`, `src/styles.css`).
- **README** usage and project-structure notes updated for the new placement.
...
- **Mobile touch optimizations removed.** Rolled back all Material 3 mobile
  overrides (custom FABs, bottom sheets, Excalidraw chrome restyling, property
  panel touch targets, lucide-react icons, Roboto font). The mobile UI now
  ships Excalidraw's default mobile experience without custom CSS overrides.
